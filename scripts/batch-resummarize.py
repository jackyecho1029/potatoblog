...
def fetch_transcript(video_id):
    ...
    if not result.stdout:
        print(f"  Transcript fetch failed: empty response", file=sys.stderr)
        return None
    try:
        data = json.loads(result.stdout)
    except json.JSONDecodeError:
        print(f"  Transcript fetch failed: invalid JSON", file=sys.stderr)
        return None
...
def main():
    ...
    for i, (video_id, author, filename) in enumerate(VIDEOS):
        ...
        # Summarize
        print(f"  Summarizing with GLM ({GLM_MODEL})...")
        summary = summarize_with_glm(author, transcript)
        if not summary:
            print(f"  FAILED: GLM error")
            failed += 1
            continue
